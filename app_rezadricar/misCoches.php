...
if (isset($_POST["btnCancelarCompra"])) {
    $url = DIR_SERV . "/borrarCompra/" . $_POST["btnCancelarCompra"];
    $resp = consumir_servicios_REST($url, "DELETE");
    if (isset($resp->error))
        die($resp->error);

    $_SESSION["accion"] = "LA COMPRA SE HA CANCELADO CON ÉXITO";
...
<body>
    ...
    <main>
        ...
        <nav id="enlacecontacto">
            <p><a onclick="location.href='index.php'">Inicio</a> &#x25B6; <small>Mis Coches</small></p>
        </nav>
        ...
        <div id="contenedorCoches">
            ...
            $consulta = "select coches.id,coches.foto,coches.marca,coches.modelo,ventas.id,ventas.fk_usuarios_id from coches,ventas where coches.id = ventas.fk_coches_id and ventas.fk_usuarios_id =" . $idUsuario;
            ...
            ?>
                <div id="productos">
                    ...
                    while ($fila = mysqli_fetch_assoc($resultado)) {
                        ...
                        echo "<form id='contenedorboton' action='misCoches.php' method='post'>";
                        echo "<button onclick='return confirm(\"¿ESTÁS SEGURO DE QUE QUIERES CANCELAR ESTA COMPRA?\");'class='sabermas' value='" . $fila["id"] . "' name='btnCancelarCompra' >CANCELAR COMPRA</button>";
                        echo "</form>";
                        echo "</div>";
                    }
                    ?>
