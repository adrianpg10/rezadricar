...
$url = DIR_SERV . "/obtenerUsuario/" . $_SESSION['id_user'];
...
if (isset($_POST["btneliminarCuenta"])) {
    ...
    $consulta = "select ventas.id from usuarios,ventas where usuarios.id = ventas.fk_usuarios_id and usuarios.id =" . $_SESSION['id_user'];
    $resultadoVentas = mysqli_query($conexion, $consulta);

    $consulta = "select pruebas.id from usuarios,pruebas where usuarios.id = pruebas.fk_usuarios_id and usuarios.id =" . $_SESSION['id_user'];
    $resultadoCitas = mysqli_query($conexion, $consulta);

    if (mysqli_num_rows($resultadoVentas) > 0) {
        $_SESSION["accion"] = "PARA ELIMINAR SU CUENTA, ELIMINE SUS COMPRAS";
        //header("Location:vista_normal.php");
    }

    if (mysqli_num_rows($resultadoCitas) > 0) {
        $_SESSION["accion"] = "PARA ELIMINAR SU CUENTA, ELIMINE SUS CITAS";
        //header("Location:vista_normal.php");
    }
    
    if((mysqli_num_rows($resultadoCitas) == 0) && (mysqli_num_rows($resultadoVentas) == 0)){

        $url = DIR_SERV . "/borrarUsuario/" . $_SESSION['id_user'];
            ...
            <div id="datosUser">
                <div id="contenidoUser">
                    ...
                    if (isset($respuesta->usuario)) {
                        ...
                        echo "<h2>El usuario con Id: " . $_SESSION['id_user'] . " ya no se encuentra registrado en la BD</h2>";
                    }
                    ?>
                ...
                <form method="post" action="vista_normal.php">

                    <input class="botonForm" type="submit" name="btneliminarCuenta" value="ELIMINAR CUENTA" />
                    <input class="botonForm" type="submit" name="misCitas" value="MIS CITAS" />
                    <input class="botonForm" type="submit" name="misCoches" value="MIS COCHES" />
